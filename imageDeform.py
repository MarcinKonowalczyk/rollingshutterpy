''' Functions for deformation of an image for use with the rollingShutter
'''
...
def deform(image, nodes):
    ...
    '''
    output = image.copy() # Take copy of the input image
    w, h = image.size
    ...
    for ri in range(0,h):
        ...
        for ci, pixel in enumerate(row):
            ...
            output.putpixel((ci,ri),tuple(np.uint8(out)))
